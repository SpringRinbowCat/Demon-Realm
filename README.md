...
- 当前已接入 cocos2d-x 4.0，macOS 可构建运行：应用入口、配置加载、主场景、进入游戏页面和战斗页面已打通，点击进入游戏可切到战斗页面。
- 战斗页面上半区展示首个 Boss 与剩余血量，下半区是可上下滑动的英雄栏，按配置展示全部英雄的等级、攻击力、攻击间隔和已解锁技能；底部栏除战斗外的四个入口只输出日志。
- 点击英雄卡片可展开英雄详情：攻击力升级、攻击速度升级、英雄介绍与技能介绍。升级已可用：攻击力增量按分段倍率累乘，攻击速度每次缩短当前攻击间隔的 1%，两者共用一条随英雄等级递增的费用序列；金币不足时花费显示为红色且点击被拒绝。技能说明里用大括号写的数值会替换成实际值。
- 自动攻击已实现：每个英雄每过一次攻击间隔造成等于攻击力的伤害，并获得等量金币，金币与 Boss 剩余血量随之刷新。所有游戏数值用字符串定点小数存储与计算（固定 4 位小数、一律向下取整），伤害与金币产出已预留 buff 修正接入点。
- 技能系统已实现点击触发类技能：点击 Boss 贴图范围会结算已解锁的点击技能，沛总的“脸滚键盘”造成一次等于攻击力的伤害，“成长”按 20% 概率永久提升攻击力（提升量为攻击力等级 × 英雄等级 ÷ 50）。概率每次点击按当前时间独立取随机数，没有保底。
- 升级、掉落、关卡推进、挂机离线收益和存档尚未实现；模块 README 中标注的目标结构不等同于已完成代码。
